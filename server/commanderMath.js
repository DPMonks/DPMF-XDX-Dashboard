...
export function bindLiveScope(balances = null, markets = null, lpEarnings = null) {
  ...
  if (balances?.ok) {
    ...
    if (balances.xdx && Number.isFinite(balances.xdx.balance)) scope.xdx = Number(balances.xdx.balance);
  ...
  if (markets?.amm?.price != null && Number.isFinite(Number(markets.amm.price))) {
    scope.xdx_mark = Number(markets.amm.price);
  }
...
export function mathNeedsWallet(raw) {
  const q = String(raw || "").toLowerCase();
  return /\b(my|me|i |wallet|balance|holding|xrp balance|xdx balance)\b/.test(q);
}
...
export function mathNeedsMarkets(raw) {
  const q = String(raw || "").toLowerCase();
  return /\b(mark|price|pool|lp|notional|share|fee|xdx|depth)\b/.test(q);
}
...
export function parseMathIntent(raw, scope = {}) {
  ...
  // percent of: "20% of 150", "what's 20% of my xrp balance"
  {
    ...
    if (m) {
      ...
      if (key in scope) {
        ...
        base = scope.xrp;
        baseLabel = "xrp";
      } else if (/xdx/.test(key) && scope.xdx != null) {
        base = scope.xdx;
        baseLabel = "xdx";
      } else {
      ...
      if (base == null && /xrp|xdx|balance|wallet/.test(q)) {
  ...
  // notional: "notional 500 xdx at 0.02" / "notional size 1000 @ 1.25"
  {
    const m = text.match(/notional(?:\s*size|\s*value)?\s*(?:of\s*)?([\d,]+(?:\.\d+)?)\s*(?:xdx|units?)?\s*(?:at|@|×|x|\*)\s*([\d,]+(?:\.\d+)?)/i);
    if (m) {
