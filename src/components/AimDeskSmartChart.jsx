...
import { defaultCexLimit, fetchCexCandles } from "../chart/cexCandles";
...
export default function AimDeskSmartChart({ deskOrders = [], estimate = null, onScenarioExplain = null }) {
  ...
  useEffect(() => {
    ...
    async function loadCex() {
      try {
        const want = defaultCexLimit(tf, historyBarsForTf(tf));
        const payload = await fetchCexCandles({ interval: tf, limit: want });
        if (cancelled) return;
        setCexCandles(payload.candles || []);
        setCexLabel(payload.label || "");
      } catch {
        if (!cancelled) {
          setCexCandles([]);
          setCexLabel("");
  ...
  }, [tf, prices, livePrice, bands.mid, now, cexCandles]);
      ...
      <div className="aim-desk-chart-head">
        <div>
          ...
          <p className="aim-desk-chart-sub">
            {cexLabel || "Candles: CEX XRP/USD | Book: XRPL XRP/RLUSD"}. Public book + desk OfferCreates. Pick Bullish or Bearish for this TF ({ESTIMATE_LABEL}).
