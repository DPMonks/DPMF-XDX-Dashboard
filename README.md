...
Public production is **https://xdx-exchange.dpmf.technology** (same Vercel production alias: `https://dpmf-xdx-dashboard.vercel.app`). No SSO. Cards and `/api/overview` are live. `DATABASE_URL` must be `postgres://…`, not the indexer HTTP host.
...
| `AIM_DESK_BASE_URL` | Vercel env (Production + Preview), server-only | aim-commander **origin** for admin realized PnL, e.g. `https://<aim-commander>.up.railway.app`. `AIM_BASE_URL` is used when this is unset. The proxy calls `GET /aim/realized-pnl/recent`, `GET /aim/realized-pnl/totals`, and `GET /api/aim/admin/pnl/summary-all`. |
